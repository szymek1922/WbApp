...
namespace WbApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    var query = "Declare @User NVARCHAR(128) = (select Id from AspNetUsers where Email = '" + user + "') " +
                                "Insert into UserLogs(UserId, Date, Action) values(@User, GETDATE(), 'Zalogowanie')";
                ...
                default:
                    user = model.Email;
                    query = "Declare @User NVARCHAR(128) = (select Id from AspNetUsers where Email = '" + user + "') " +
                                "Insert into UserLogs(UserId, Date, Action) values(@User, GETDATE(), 'Błędne logowanie')";
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var query = "Declare @User NVARCHAR(128) = (select Id from AspNetUsers where Email = '" + user.Email + "') " +
                   "Insert into UserLogs(UserId, Date, Action) values(@User, GETDATE(), 'Rejestracja')";
        ...
        public ActionResult LogOff()
        {
            ...
            var user = AuthenticationManager.User.Identity.GetUserName();
            var query = "Declare @User NVARCHAR(128) = (select Id from AspNetUsers where Email = '" + user + "') " +
                        "Insert into UserLogs(UserId, Date, Action) values(@User, GETDATE(), 'Wylogowanie')";
